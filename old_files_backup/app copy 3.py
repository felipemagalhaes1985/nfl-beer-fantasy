...
class Jogo(db.Model):
    ...
    # Relações
    palpites = db.relationship('Palpite', backref='jogo', lazy=True, cascade="all, delete-orphan") # Adicionado cascade para deletar palpites
...
def home():
    ...
    # 1. Encontrar a menor semana que ainda possui jogos não finalizados.
    # 2. Se todos os jogos estiverem finalizados, exibir a maior semana existente.
    # 3. Se não houver jogos no DB, exibir Semana 1.
...
def forgot_password():
    ...
    return render_template('forgot_password.html') # Renderiza forgot_password.html
            ...
            if jogo:
                try:
                    ...
                    if todos_jogos_semana_finalizados(semana_do_jogo_excluido):
                        ...
                        # Se um jogo é excluído e a semana não estava toda finalizada,
                        # talvez os pontos F1-like daquela semana devam ser recalculados ou zerados se eles existiam
                        # e agora não há mais base para eles.
                        # Por simplicidade, recalculamos só se a semana *ainda* estiver completa.
                        # Para garantir a precisão, pode-se forçar um recálculo se a semana já teve F1 points
                        # mas agora está incompleta.
                        pass 
...
# --- Rotas placeholder (ainda não implementadas) ---
@app.route('/admin_config')
@login_required
@admin_required
def admin_config():
    flash('Funcionalidade de Gerenciar Semana Atual ainda não implementada.', 'info')
    return render_template('admin_config.html') # Assegure-se de que este template existe

@app.route('/adicionar_participante_admin', methods=['GET', 'POST']) # Nova rota para adicionar participante via admin
@login_required
@admin_required
def adicionar_participante_admin():
    flash('Funcionalidade de Adicionar Novo Participante (Admin) ainda não implementada.', 'info')
    # No futuro, aqui você renderizaria um formulário para o admin criar um participante
    return redirect(url_for('admin_dashboard'))

@app.route('/gerenciar_participantes')
@login_required
@admin_required
def gerenciar_participantes():
    flash('Funcionalidade de Ver/Editar/Excluir Participantes ainda não implementada.', 'info')
    return redirect(url_for('admin_dashboard'))

@app.route('/calcular_f1_rodada/<int:numero_rodada>')
@login_required
@admin_required
def calcular_f1_rodada(numero_rodada):
    flash(f'Funcionalidade de Calcular Pontos F1 para rodada {numero_rodada} ainda não implementada.', 'info')
    return redirect(url_for('admin_dashboard'))

@app.route('/reset_password/<token>', methods=['GET', 'POST'])
def reset_password(token):
    flash('Funcionalidade de redefinição de senha com token ainda não implementada.', 'info')
