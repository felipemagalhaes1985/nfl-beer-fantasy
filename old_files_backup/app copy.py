...
def calcular_pontos_f1_por_rodada(semana):
    ...
    if pontuacoes_raw_por_participante:
        ...
        for i, (participante_id, nome, total_pontos_raw) in enumerate(pontuacoes_raw_por_participante):
            ...
            # Atualiza ou cria o registro de PontuacaoRodada
...
# --- Rotas placeholder para o admin_dashboard ---
...
@admin_required
def adicionar_jogo():
    flash('Funcionalidade de Adicionar Jogo Manualmente ainda não implementada.', 'info')
    return redirect(url_for('admin_dashboard'))

@app.route('/gerenciar_jogos')
@login_required
@admin_required
def gerenciar_jogos():
    flash('Funcionalidade de Ver/Editar/Excluir Jogos ainda não implementada.', 'info')
    return redirect(url_for('admin_dashboard'))
