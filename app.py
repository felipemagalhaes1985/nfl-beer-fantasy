...
def meu_perfil():
    ...
    # NOVO: Busca o histórico de pontuações F1-like semanais do usuário
    historico_pontuacao_semanal = PontuacaoRodada.query.filter_by(participante_id=current_user.id).order_by(PontuacaoRodada.semana.asc()).all()

    return render_template('meu_perfil.html', 
                           palpites=palpites_do_usuario,
                           total_pontos_f1=total_pontos_f1,
                           historico_pontuacao_semanal=historico_pontuacao_semanal) # Passa o histórico para o template
...
# --- Rota para a página de Evolução da Pontuação (HTML) ---
@app.route('/evolucao_pontuacao')
def evolucao_pontuacao_page():
    return render_template('evolucao_pontuacao.html')

# --- Rota para a API de dados do gráfico ---
@app.route('/api/evolucao_pontuacao_data')
def evolucao_pontuacao_data():
    # 1. Obter todas as semanas que possuem pontuações registradas, ordenadas
    todas_semanas = db.session.query(PontuacaoRodada.semana).distinct().order_by(PontuacaoRodada.semana).all()
    labels = [f"Semana {s.semana}" for s in todas_semanas]
    semanas_numericas = [s.semana for s in todas_semanas]

    # 2. Obter todos os participantes que pontuaram em alguma semana
    participantes_com_pontuacao = db.session.query(Participante).join(PontuacaoRodada).distinct().all()
    
    datasets = []
    
    # Cores pré-definidas para os gráficos
    # Você pode expandir esta lista se tiver muitos participantes
    # ou gerar cores dinamicamente
    colors = [
        ...
        'rgba(83, 109, 254, 1)', # Azul claro
        'rgba(255, 222, 173, 1)',# Pêssego
        'rgba(0, 128, 0, 1)'     # Verde escuro
    ]
    color_index = 0

    for participante in participantes_com_pontuacao:
        pontuacoes_por_participante = PontuacaoRodada.query.filter_by(participante_id=participante.id).order_by(PontuacaoRodada.semana).all()
        
        pontuacoes_semanais_dict = {p.semana: p.pontos_f1 for p in pontuacoes_por_participante}
        
        cumulative_scores = []
        current_sum = 0
        
        for semana_num in semanas_numericas:
            # Adiciona os pontos da semana atual, se existirem, senão 0
            current_sum += pontuacoes_semanais_dict.get(semana_num, 0)
            cumulative_scores.append(current_sum)
        
        datasets.append({
            'label': participante.nome,
            'data': cumulative_scores,
            'borderColor': colors[color_index % len(colors)], # Atribui uma cor da lista
            'fill': False,
